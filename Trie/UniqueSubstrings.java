...
public class UniqueSubstrings {
    ...
    public static  int countNode (Node root) {
     ...
     int count = 0;
     for (int i =0; i < 26; i++) { // Traverse through all children
        if (root.children[i] != null) { // If the child node exists, recursively count its children
            count += countNode(root.children[i]); // Add the count of children to the total count
        }
     }
     return count+1; // Return the count of the current node plus the count of its children
    }
